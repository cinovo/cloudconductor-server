...
function enableCheckboxToggle() {
    ...
    $("input[behavior=toggle-exclusive]").click(
        function() {
            ...
            if (isSelected) {
                element.closest("form").find(
                    "input:checkbox[value=" + $(this).prop("value") + "]").filter(
                    function() {
                        return (this.name != element.prop("name")) && !this.disabled;
...
function initDropDown() {
      $('.dropdown-area').on('show.bs.collapse', function () {
    	  $(this).parent().find(".toggle-dropdown").toggleClass('glyphicon-chevron-down glyphicon-chevron-up');
      });
      $('.dropdown-area').on('hide.bs.collapse', function () {
    	  $(this).parent().find(".toggle-dropdown").toggleClass('glyphicon-chevron-up glyphicon-chevron-down');
      });
}
            ...
            success: function(data) {
                ...
                if ($("#popModal").hasClass('in')) {
                    if (reloaded.info || data.info)
                        ...
                        .find('.modal-body')
                        .prepend(
                            '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' + reloaded.info + '</div>');
                }
                return 0;
            },
            ...
            $.fn.dataTableExt.afnFiltering
                .push(function(oSettings, aData, iDataIndex) {
                    ...
                    var objectDate = aData[iStartDateCol]
                        .substring(6, 10) + aData[iStartDateCol].substring(3, 5) + aData[iStartDateCol].substring(0, 2);
