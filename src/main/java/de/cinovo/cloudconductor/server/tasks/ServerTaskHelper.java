...
public class ServerTaskHelper implements SchedulingConfigurer, IServerRepoTaskHandler {
	...
	private final Map<String, AbstractTrigger> running = new HashMap<>();
	...
	public void newRepo(ERepo repo) {
		List<ERepoMirror> mirrors = this.repoMirrorDAO.findForRepo(repo);
		if (!mirrors.isEmpty() && this.indexTaskType.equals("single")) {
			EServerOptions settings = this.optionsDAO.get();
