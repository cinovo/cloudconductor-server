...
public class ServiceDefaultStateDAOHib extends EntityDAOHibernate<EServiceDefaultState, Long> implements IServiceDefaultStateDAO {
	...
	@Override
	public EServiceDefaultState findByServiceAndTemplate(Long serviceId, Long templateId) {
		// language=HQL
		return this.findByQuery("FROM EServiceDefaultState as ss WHERE ss.serviceId = ?1 AND ss.templateId = ?2", serviceId, templateId);
	}
	
	@Override
	public List<EServiceDefaultState> findByTemplate(Long templateId) {
		// language=HQL
		return this.findListByQuery("FROM EServiceDefaultState AS ss WHERE ss.templateId = ?1", templateId);
	}
}
